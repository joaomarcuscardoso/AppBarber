...
const DatePrevArea = styled.View`
  flex:1;
  justify-content:center;
  align-items:center;
`;
...
export default ({show, setShow, user, service}) => {
  ...
  useEffect(() => {
    if(user.available) {
      ...
      for(let i=1;i<daysInMonth;i++) {
        ...
        console.log("filters"+user.available);
        newListDays.push({
  ...
  const handleRightDateClick = () => {
    ...
    //setSelectedHour(mountDate.getHours());
    setSelectedDay(0);
  }
    ...
    >
      <ModalArea>
        <ModalBody>
          ...
          <ModalItem>
            <DateInfo>
              <DatePrevArea onPress={handleRightDateClick}>
                <NavPrevIcon width="35" height="35" fill="#000000" />
              </DatePrevArea>
              <DateTitleArea>
                <DateTitle>{months[selectedMonth]} {selectedYear}</DateTitle>
              </DateTitleArea>
              <DateNextArea onPress={handleLeftDateClick}>
